...
In this tutorial we will use the MTA to analyse a legacy Java application to determine it's suitability for migration to OpenShift.
...
As cluster administrator, navigate to the "Operators" -> "OperatorHub".
...
Go to the "Installed Operators" page and click on "Migration Toolkit for Applications Operator".
...
You will be presented with the default landing page of the MTA, from here you can create a project and start analyzing your applications.
...
You will now be presented with options for packages. In order to accelerate the analysis, we will choose only the business packages related to our application. In the case of this application, these classes belong to the com.redhat package.
...
Once the report is finished, click image:mta-icon.png[] to access the report. Click on addressbook.war application.

image::mta-report.png[]

The analysis of the sample application shows no mandatory issues to resolve in order for this application to run in a cloud environment. 
