...
In this section we're going to deploy a sample JBoss EAP application which connects to a Postgres database.  
...
Before we build and deploy our application we're going to deploy the required postgres database.  

To do this, login to OpenShift as a developer and switch to the developer-ui.  From the developer-ui go to "+Add" and then select "Database", choose the "PostgreSQL" template and then click on "Instantiate Template".  Fill out the form as shown below:
...
In traditional JBoss EAP deployments this would be done by running jboss.cli commands or editing configuration (e.g. standalone.xml).  When deploying on OpenShift the S2I process will perform these configurations for us.
